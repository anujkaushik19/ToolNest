...
export function WordToPdf() {
  ...
  const onFiles = useCallback((files: File[]) => {
    const doc = files.find((f) => ALLOWED.test(f.name));
    if (!doc) {
      setError("Please choose a Word, Excel, PowerPoint or OpenDocument file.");
      return;
    }
    setError(null);
    setResultUrl((prev) => {
  ...
  const convert = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(`${API}/api/convert/office`, {
        method: "POST",
        body: form,
      });
      if (!resp.ok) {
  ...
  const downloadName = (file?.name.replace(/\.[^.]+$/, "") || "document") + ".pdf";
    ...
    return (
      <div className="space-y-4">
        ...
        <Dropzone
          accept={ACCEPT}
          onFiles={onFiles}
          hint="Word · Excel · PowerPoint · OpenDocument → PDF"
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 rounded-2xl glass p-4">
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-indigo-500">
      ...
      {resultUrl ? (
        ...
        >
          {busy ? (
            ...
            <>
              <Sparkles className="h-5 w-5" /> Convert to PDF
            </>
          )}
        </button>
      )}
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-3 text-sm text-amber-100/90">
      ...
      <p>
        ...
        converted, and immediately discarded — it is never stored. All our other
